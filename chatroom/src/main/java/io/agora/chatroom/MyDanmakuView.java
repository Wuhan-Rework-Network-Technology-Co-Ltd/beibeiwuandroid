...
//                .setCacheStuffer(new BackgroundCacheStuffer(), null)  // 绘制背景使用BackgroundCacheStuffer
     ...
     * 发送自定义弹幕
     */
    public void addDanmakuWithDrawable(String myWords, @DrawableRes int id) {
...
//        danmaku.text = "这是一条弹幕";
        danmaku.text = createSpannable(drawable,myWords);
//        danmaku.padding = 5;
        danmaku.priority = 0;  // 可能会被各种过滤器过滤并隐藏显示
        danmaku.isLive = false;
        danmaku.setTime(getCurrentTime() + 1200);
...
//        danmaku.borderColor = Color.GREEN;
    ...
    private SpannableStringBuilder createSpannable(Drawable drawable,String myWords) {
        String text = "bitmap";
        SpannableStringBuilder spannableStringBuilder = new SpannableStringBuilder(text);
        CenteredImageSpan span = new CenteredImageSpan(drawable);//ImageSpan.ALIGN_BOTTOM);
        spannableStringBuilder.setSpan(span, 0, text.length(), Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
        spannableStringBuilder.append(myWords);
