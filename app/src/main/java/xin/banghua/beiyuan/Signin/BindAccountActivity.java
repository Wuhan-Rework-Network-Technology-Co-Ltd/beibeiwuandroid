    ...
    public void puzzle(){
        ...
        captcha.setCaptchaListener(new Captcha.CaptchaListener() {
            ...
            public String onAccess(long time) {
                Toast.makeText(BindAccountActivity.this,"验证成功",Toast.LENGTH_SHORT).show();
                OkHttpInstance.sendCode(userAccountString, new OkHttpResponseCallBack() {
                    @Override
                    public void getResponseString(String responseString) {
                        Toast.makeText(mContext, "验证码发送成功", Toast.LENGTH_LONG).show();
            ...
            public String onFailed(int failedCount) {
                Toast.makeText(BindAccountActivity.this,"验证失败",Toast.LENGTH_SHORT).show();
            ...
            public String onMaxFailed() {
                Toast.makeText(BindAccountActivity.this,"验证超过次数，你的帐号被封锁",Toast.LENGTH_SHORT).show();
                return "验证失败,帐号已封锁";
            }
        });
    }
            ...
            public void inputComplete() {
                if (codeView.getInputContent().length()==4){
                    if (codeView.getInputContent().equals(smscode)){
                        OkHttpInstance.bindPhone(userAccountString, new OkHttpResponseCallBack() {
                            @Override
                            public void getResponseString(String responseString) {
                                Toast.makeText(BindAccountActivity.this,"绑定成功！",Toast.LENGTH_SHORT).show();
                                Common.userInfoList.setPhone(userAccountString);
                                finish();
                            }
                        });
                    }else {
                        Toast.makeText(BindAccountActivity.this,"验证码错误，请重新输入",Toast.LENGTH_SHORT).show();
